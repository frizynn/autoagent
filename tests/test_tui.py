...
class TestSparkline:
    ...
    def test_constant(self) -> None:
        ...
        assert len(result) == 3
        # All same value — all should be the same char
...
async def test_tui_processes_events() -> None:
    ...
    async with app.run_test(size=(120, 40)) as pilot:
        ...
        # Give the poll timer a chance to drain
        await pilot.pause()
        app._poll_events()  # force a drain
        await pilot.pause()
        ...
        assert app.best_iteration_id == "1"
        assert len(app._scores) == 1
        assert app._scores[0] == 0.75
...
async def test_tui_quit_binding() -> None:
    ...
    async with app.run_test(size=(120, 40)) as pilot:
        await pilot.press("q")
        # App should be closing — return_code gets set on exit
        assert True  # quit binding fires without error



async def test_tui_with_loop_factory() -> None:
    """A loop_factory should be invoked in the background."""
    call_log: list[str] = []
